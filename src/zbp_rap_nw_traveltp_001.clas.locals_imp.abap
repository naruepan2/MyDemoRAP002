...
CLASS lhc_travel IMPLEMENTATION.
  ...
  METHOD earlynumbering_create.
    ...
    DATA:
      entity           TYPE STRUCTURE FOR CREATE zi_rap_nw_traveltp_001,
    ...
    ELSE.
      ...
      SELECT SINGLE FROM zrap_nw_atrav001 FIELDS MAX( travel_id ) AS travelID INTO @travel_id_max.
      "Get max travel ID from draft table
      SELECT SINGLE FROM zrap_nw_dtrav001 FIELDS MAX( travelid ) INTO @DATA(max_travelid_draft).
  ...
  METHOD setStatusToOpen.
    ...
    READ ENTITIES OF zi_rap_nw_traveltp_001 IN LOCAL MODE
     ENTITY Travel
    ...
    MODIFY ENTITIES OF zi_rap_nw_traveltp_001 IN LOCAL MODE
      ENTITY Travel
*        UPDATE SET FIELDS
  ...
  METHOD validateCustomer.
    ...
    DATA ls_failed TYPE STRUCTURE FOR FAILED LATE zi_rap_nw_traveltp_001\\travel.
    DATA ls_reported TYPE STRUCTURE FOR REPORTED LATE zi_rap_nw_traveltp_001\\travel.
...
*                TO reported-travel.
      ...
      ls_reported = VALUE #( %tky = travel-%tky
                             %state_area = 'VALIDATE_CUSTOMER'
                           ).
      APPEND ls_reported TO reported-travel.

      IF travel-CustomerID IS INITIAL.
        ls_failed = VALUE #( %tky = Travel-%tky ).
      ...
      ELSEIF travel-CustomerID IS NOT INITIAL
         AND NOT line_exists( valid_customers[ customer_id = travel-CustomerID ] ).
        ls_failed = VALUE #( %tky = Travel-%tky ).
        APPEND ls_failed TO failed-Travel.
  ...
  METHOD deductDiscount.
    DATA travels_for_update TYPE TABLE FOR UPDATE zi_rap_nw_traveltp_001.
    ...
    LOOP AT keys_with_valid_discount ASSIGNING FIELD-SYMBOL(<key_with_valid_discount>)
     WHERE %param-discount_percent IS INITIAL
        OR NOT %param-discount_percent BETWEEN 1 AND 100.
...
*                    ) TO travels_for_update.
  ...
  METHOD copyTravel.
    "%is_draft = 00 : New instance must be created as active instance
    "     - i.e., must be persisted.
    "%is_draft = 01 : New instance must be created as draft instance
    "     - i.e., will first be only stored in the draft table.

    DATA:
      travels TYPE TABLE FOR CREATE zi_rap_nw_traveltp_001\\Travel.
...
*                                                          THEN if_abap_behv=>fc-o-disabled ELSE if_abap_behv=>fc-o-enabled   )
                        ...
                        "   TEST
                        """"""""""""""""""""""""""""""""""""""""""""""
                        %features-%action-Edit          = COND #( WHEN travel-OverallStatus = travel_status-accepted
                                                                    THEN if_abap_behv=>fc-o-disabled
                                                                    ELSE if_abap_behv=>fc-o-enabled   )
                        %features-%action-acceptTravel  = COND #( WHEN travel-OverallStatus = travel_status-accepted
                                                                    THEN if_abap_behv=>fc-o-disabled
                                                                    ELSE if_abap_behv=>fc-o-enabled   )
                        %features-%action-rejectTravel  = COND #( WHEN travel-OverallStatus = travel_status-rejected
                                                                    THEN if_abap_behv=>fc-o-disabled
                                                                    ELSE if_abap_behv=>fc-o-enabled   )
                     ) ).
